...
def load_adr_commands(self, _):
    ...
    # Credential commands
    with self.command_group("iot adr ns credential", command_type=adr_credential_ops, is_preview=True) as cmd_group:
    ...
    # Policy commands
    with self.command_group("iot adr ns policy", command_type=adr_policy_ops, is_preview=True) as cmd_group:
